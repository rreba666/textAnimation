    ...
    <div className="card p-4 h-full flex flex-col">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setVd(new Date(yr, mo - 1, 1))} className="p-1 rounded-lg hover:bg-notebook-bg text-text-secondary"><ChevronLeft size={16} /></button>
          <span className="text-sm font-medium text-text-primary w-28 text-center">{yr}年 {mo + 1}月</span>
          <button onClick={() => setVd(new Date(yr, mo + 1, 1))} className="p-1 rounded-lg hover:bg-notebook-bg text-text-secondary"><ChevronRight size={16} /></button>
        </div>
      </div>
        ...
        {days.map((d, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center py-0.5">
              <div className={`w-7 h-7 flex items-center justify-center text-xs rounded-full relative ${td ? 'bg-warm-orange text-white font-semibold' : inM ? 'text-text-primary hover:bg-notebook-bg' : 'text-text-light'}`}>{format(d, 'd')}</div>
