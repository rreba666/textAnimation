...
RE:序章是一个纯前端个人仪表盘应用，所有数据通过 Zustand persist 中间件自动同步到 localStorage。涵盖天气、待办、习惯打卡、Markdown 笔记、快捷链接、番茄钟、日历、每日一言等 15 个功能模块。
...
| 习惯打卡 | 预设 + 自定义 + 爱心/星星/时钟打卡（补签标识）+ Recharts 月统计 + 连续天数（补签不计入）+ 全年热力图弹窗 |
...
| 治愈语录 | 右下角彩蛋，10 句随机语录 |
| 交互动画 | GSAP 驱动：打卡弹性缩放、待办滑入淡入、快捷键脉冲反馈、天气微动画 |
| 光斑背景 | CSS 光斑漂移 + 波动渐变（亮色阳光/暗色月光） |
| 卡片装饰 | 书脊阴影、装订孔、和纸胶带、图钉、折角、纸张堆叠立体感 |
...
└── components/
    ...
    ├── AchievementsModal.tsx   # 成就徽章弹窗（10 个徽章 + 稀有度 + 勋章墙）
    ├── AchievementNotify.tsx   # 成就解锁通知（Steam 风格 + Web Audio）
    ├── GuideTour.tsx           # 首次访问引导遮罩（聚光灯 + 分步介绍）
